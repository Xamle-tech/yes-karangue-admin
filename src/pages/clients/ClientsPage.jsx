...
import { fetchClients, fetchClientById, deleteClient } from '../../services/clientsService';
...
export default function ClientsPage() {
  ...
  const handleAddClient = async (formData) => {
    // Here we would call the create/update API
    // For now we simulate success and close form
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      setShowForm(false);
      ...
      setToast({
        message: editingClient ? 'Client mis à jour avec succès' : 'Client créé avec succès',
        type: 'success'
      });
  ...
  const handleDeleteClient = async () => {
    ...
    try {
      ...
      if (viewingClient && viewingClient.id === deleteModal.clientId) {
    ...
    if (viewingClient) {
      return (
        <ClientDetails
          ...
          onDelete={(client) => confirmDeleteClient(client)}
        />
      );
    }
    ...
    <div className="space-y-6">
      ...
      {deleteModal.show && (
        <ConfirmationModal
          ...
          message={`Êtes-vous sûr de vouloir supprimer le client ${deleteModal.clientName} ?`}
          confirmText="Supprimer"
          cancelText="Annuler"
          onConfirm={handleDeleteClient}
          onCancel={() => setDeleteModal({ show: false, clientId: null, clientName: '' })}
